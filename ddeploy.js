...
page.open(SITE.config.url, function (status) {
	...
	if (status !== "success") {
		...
		filterFile = function(){
			...
			console.info('Filtering by file name ', SITE.config.components[current_component].remote);

			var inputFilter = page.evaluate(function(SITE, current_component, keyUp) {

				var inputFilter = $('.x-toolbar.x-small-editor.k-entity-tab.x-toolbar-layout-ct input').val(SITE.config.components[current_component].remote);
				keyUp($('.x-toolbar.x-small-editor.k-entity-tab.x-toolbar-layout-ct input')[0]);

				return inputFilter;//$('.x-panel-body.k-entity-tab.x-panel-body-noheader.x-panel-body-noborder span:contains('+SITE.config.components[current_component].remote+')').length == 1;

			}, SITE, current_component, keyUp);
			...
			// page.render('screenshot_filter.png');

			waitFor(function() {

				return page.evaluate(function(SITE, current_component) {
					return $('.x-panel-body.k-entity-tab.x-panel-body-noheader.x-panel-body-noborder span:contains('+SITE.config.components[current_component].remote+')').length == 1;
		...
		uploadtest = function() {
			...
			// page.render('screenshot_'+screenshot+'.png'); screenshot++;
			// page.render('screenshot_upload.png');
